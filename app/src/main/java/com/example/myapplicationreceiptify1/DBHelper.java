...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public Boolean checkusernamepasswordemailmobile(String username, String password, String email, String mobile) {

        SQLiteDatabase MyDB = this.getWritableDatabase();
        Cursor cursor = MyDB.rawQuery("Select * from users where username=? and password=? and email=? and mobile = ?", new String[]{username,password,email,mobile});
        if(cursor.getCount()>0)
            return true;
        else return false;
    }
